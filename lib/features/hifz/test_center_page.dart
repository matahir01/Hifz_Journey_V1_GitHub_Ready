...
class _TestCenterPageState extends State<TestCenterPage> {
  ...
  Future<void> _ayahRange() async {
    ...
    if (m == null) return;
    final quran = context.read<QuranRepository>();
    final first = await quran.ayahByReference(
      int.parse(m.group(1)!),
      int.parse(m.group(2)!),
    );
    final last = await quran.ayahByReference(
      int.parse(m.group(3)!),
      int.parse(m.group(4)!),
    );
    ...
    await _openAyahs(
      await quran.ayahsBetween(start, end),
      title: 'Ayah range test',
    );
  }
  ...
  Future<void> _pageTest() async {
        ...
        builder: (context, setState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              SegmentedButton<int>(
                segments: const [
                  ButtonSegment(value: 1, label: Text('1 page')),
                  ButtonSegment(value: 2, label: Text('2 pages')),
                ],
                selected: {count},
            ...
            FilledButton(
              onPressed: () {
                ...
                if (page != null && page >= 1 && page <= 604) {
                  Navigator.pop(context, (page, count));
                }
              },
    ...
    final ayahs =
        await context.read<QuranRepository>().ayahsForPages(result.$1, result.$2);
    await _openAyahs(
      ayahs,
      title:
          'Mushaf pages ${result.$1}${result.$2 == 1 ? '' : '–${result.$1 + result.$2 - 1}'}',
    );
  }
                ...
                Wrap(
                  ...
                  children: [
                    for (final entry in const [
                      ...
                      ChoiceChip(
                        label: Text(entry.$2),
                        selected: pages == entry.$1,
                        onSelected: (_) =>
                            setState(() => pages = entry.$1),
                      ),
                  ],
                ),
              ],
            ...
            actions: [
              ...
              FilledButton(
                onPressed: () {
                  final page = int.tryParse(pageController.text.trim());
                  if (page != null && page >= 1 && page <= 604) {
                    Navigator.pop(context, (page, pages, label));
                  }
                },
                child: const Text('Start test'),
              ),
  ...
  Future<void> _continueAyah() async {
    ...
    final current = await context
        ...
        .ayah(ids[Random().nextInt(ids.length)]);
    if (current == null) return;
    final previous = current.id > 1
        ? await context.read<QuranRepository>().ayah(current.id - 1)
        : null;
    await _openAyahs(
      [current],
      title: 'Continue the recitation',
      cue: previous?.textUthmani ?? 'ابدأ من الآية المطلوبة',
    );
  }
  ...
  Future<void> _middle() async {
    ...
    if (words.length < 4) {
      return _openAyahs([current], title: 'Start from the middle');
    }
    final split = (words.length / 2).floor();
    await _openAyahs(
      [current],
      title: 'Start from the middle',
      cue: words.take(split).join(' '),
      expected: words.skip(split).join(' '),
    );
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          _ModeTile(
            ...
            title: 'Ayah range',
            subtitle: 'Example: Al-Baqarah 2:1–2:5',
            onTap: _ayahRange,
          ),
          _ModeTile(
            icon: Icons.menu_book_rounded,
            title: 'Mushaf page(s)',
            subtitle: 'One or two consecutive pages',
            onTap: _pageTest,
          ),
          _ModeTile(
            icon: Icons.grid_view_rounded,
            title: 'Hizb portion',
            subtitle: 'Thumun, quarter, half or full Hizb',
            onTap: _hizbTest,
          ),
          ...
          _ModeTile(
            ...
            subtitle: 'A short surprise passage',
            onTap: _random,
          ),
          _ModeTile(
            icon: Icons.warning_amber_rounded,
            title: 'Weak ayahs',
            subtitle: 'Focus on weaker recall',
            onTap: _weak,
          ),
          _ModeTile(
            icon: Icons.repeat_rounded,
            title: 'Revision due',
            subtitle: 'Test what is due for revision',
            onTap: _due,
          ),
          _ModeTile(
            icon: Icons.link_rounded,
            title: 'Continue from a cue',
            subtitle: 'See a cue, then continue from memory',
            onTap: _continueAyah,
          ),
          _ModeTile(
            icon: Icons.segment_rounded,
            title: 'Start from the middle',
            subtitle: 'Complete the rest of the ayah',
            onTap: _middle,
          ),
        ],
      ),

...
class SettingsPage extends StatelessWidget {
              ...
              children: [
                ...
                ListTile(
                  ...
                  onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => const AudioLibraryPage(),
                    ),
                  ),
                ),
              ],
                  ...
                  onTap: () async {
                    final ok = await showDialog<bool>(
                      ...
                      builder: (c) => AlertDialog(
                        title: const Text('Restore backup?'),
                        content: const Text(
                          'Current Hifz progress and history will be replaced by the selected backup.',
                        ),
                        actions: [
                          TextButton(
                    ...
                    if (ok == true && context.mounted) {
                      try {
                        final restored =
                            await context.read<BackupService>().restoreBackup();
  ...
  Future<void> _chooseTarget(
          ...
          Widget choice(
            ...
            int? fixed,
              ...
              ChoiceChip(
                label: Text(label),
                selected:
                    unit == value && (fixed == null || amount == fixed),
                onSelected: (_) => setState(() {
                  unit = value;
                  if (fixed != null) amount = fixed;
                }),
              );
          return SafeArea(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    runSpacing: 8,
                    children: [
                      choice('3 ayahs', HifzTargetUnit.ayahs, fixed: 3),
                      choice('4 ayahs', HifzTargetUnit.ayahs, fixed: 4),
                      choice('5 ayahs', HifzTargetUnit.ayahs, fixed: 5),
                      choice('1 page', HifzTargetUnit.pages, fixed: 1),
                      choice('2 pages', HifzTargetUnit.pages, fixed: 2),
                      choice('Thumun', HifzTargetUnit.thumun),
                      choice('¼ Hizb', HifzTargetUnit.quarterHizb),
                      choice('½ Hizb', HifzTargetUnit.halfHizb),
                      choice('1 Hizb', HifzTargetUnit.hizb),
                    ],
                  ),
                  if (unit == HifzTargetUnit.ayahs) ...[
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: Slider(
                            value: amount.clamp(1, 20).toDouble(),
                            min: 1,
                            max: 20,
                            divisions: 19,
                            label: '$amount',
                            onChanged: (v) =>
                                setState(() => amount = v.round()),
                          ),
                        ),
                        SizedBox(width: 34, child: Text('$amount')),
                      ],
                    ),
                  ],
                  const SizedBox(height: 18),
                  FilledButton(
                    onPressed: () => Navigator.pop(context, (unit, amount)),
    ...
    if (result != null) {
      await controller.setHifzTarget(result.$1, result.$2);
    }
  }
  ...
  Future<void> _confirmReset(BuildContext context) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (c) => AlertDialog(
        title: const Text('Reset Hifz progress?'),
        content: const Text(
          'This clears memorization strength, revision history, and test results.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(c, false),
